...
export class HomeComponent implements OnInit {
  ...
  convertStringToDate(d:Date){
    ...
    $('.upcoming_movies p').text(date.toLocaleDateString("en-En", {month:'short', day:'numeric'}));
    $('.movies_opening_this_week p').text(date.toLocaleDateString("en-En", {month:'short', day:'numeric'}));

  }

  convertNumberToMillion(n:any){
    $('#top_box_office p').text("$"+this.numberInMillion(n));

  }

  numberInMillion(labelValue) {
